...
function Dashboard({items}:{items:Item[];people:Person[];tools:Tool[];movements:Movement[];toolMovements:ToolMovement[]}){const lowItems=items.filter(x=>Number(x.safety_stock)>0&&Number(x.stock)<Number(x.safety_stock));return <div className="content"><div className="panel"><div className="panelTitle"><b>庫存提醒</b><span>{lowItems.length?`有 ${lowItems.length} 項低於安全庫存`:'目前沒有低庫存品項'}</span></div>{lowItems.length?<Table rows={lowItems.map(x=><tr key={x.id}><td>{x.name}</td><td>{x.spec||'-'}</td><td>{x.stock} {x.unit}</td><td>{x.safety_stock} {x.unit}</td></tr>)} headers={['品項','規格','目前庫存','安全庫存']}/>:<div className="empty">目前庫存狀況正常</div>}</div></div>}
...
const inventoryHeaders=['名稱','規格','類別','單位','庫存','安全庫存','位置','備註']
async function readInventoryFile(file:File){const lower=file.name.toLowerCase();let workbook:XLSX.WorkBook;if(lower.endsWith('.csv')){workbook=XLSX.read(await readCsvText(file),{type:'string'})}else if(lower.endsWith('.xlsx')||lower.endsWith('.xls')){workbook=XLSX.read(await file.arrayBuffer(),{type:'array'})}else throw new Error('請選擇 Excel（.xlsx／.xls）或 CSV 檔案');const sheet=workbook.Sheets[workbook.SheetNames[0]];if(!sheet)throw new Error('檔案中找不到工作表');const rows=XLSX.utils.sheet_to_json<any[]>(sheet,{header:1,defval:'',raw:false});if(rows.length<2)throw new Error('檔案內沒有可匯入的資料');const headers=rows[0].map(x=>String(x).replace(/^\ufeff/,'').trim());const indexes=inventoryHeaders.map(name=>headers.indexOf(name));if(indexes[0]<0)throw new Error('找不到「名稱」欄位，請使用下載的 Excel 範本');return rows.slice(1).map(row=>indexes.map(i=>i<0?'':String(row[i]??'').trim())).filter(row=>row[0])}
function downloadInventoryTemplate(filename:string,example:string[]){const sheet=XLSX.utils.aoa_to_sheet([inventoryHeaders,example]);sheet['!cols']=[18,16,16,10,10,12,14,24].map(w=>({wch:w}));const workbook=XLSX.utils.book_new();XLSX.utils.book_append_sheet(workbook,sheet,'庫存匯入');XLSX.writeFile(workbook,filename)}
function ItemsPage({items,onDone}:{items:Item[];onDone:()=>void}){
  ...
  const rows=items.filter(x=>(x.name+x.spec+x.category).toLowerCase().includes(q.toLowerCase()))
  const template=()=>downloadInventoryTemplate('文具批次新增範本.xlsx',['原子筆','0.5mm','書寫用品','支','100','20','A-01','藍色'])
  const importFile=async(e:React.ChangeEvent<HTMLInputElement>)=>{const file=e.target.files?.[0];if(!file)return;setMsg('');try{const rows=await readInventoryFile(file);const data=rows.map(c=>({name:c[0],spec:c[1]||'',category:c[2]||'',unit:c[3]||'個',stock:Number(c[4]||0),safety_stock:Number(c[5]||0),location:c[6]||'',note:c[7]||''}));const user=(await supabase.auth.getUser()).data.user;const {error}=await supabase.from('items').insert(data.map(x=>({...x,owner_id:user!.id})));setMsg(error?error.message:`已批次新增 ${data.length} 筆文具`);if(!error)onDone()}catch(error:any){setMsg(error.message||'檔案無法讀取')}finally{e.target.value=''}}
  const removeSelected=async()=>{if(!selected.length)return;if(!window.confirm(`確定刪除已勾選的 ${selected.length} 筆文具？`))return;const {error}=await supabase.from('items').delete().in('id',selected);setMsg(error?error.message:`已刪除 ${selected.length} 筆文具`);if(!error){setSelected([]);onDone()}}
...
function MovementsPage({movements,toolMovements}:{movements:Movement[];toolMovements:ToolMovement[]}){const rows=[...movements.map(x=>({date:x.date,cat:'文具',item:x.item_name,spec:'',qty:x.quantity,person:x.person,dept:x.department,action:x.type==='out'?'領用':'入庫',note:x.note,id:'s'+x.id})),...toolMovements.map(x=>({date:x.date,cat:'個人工具',item:x.tool_name,spec:x.serial,qty:x.quantity||1,person:x.person,dept:x.department,action:x.type==='out'?'領用':'歸還',note:x.note,id:'t'+x.id}))].sort((a,b)=>b.date.localeCompare(a.date));const [q,setQ]=useState('');const filtered=rows.filter(x=>(x.item+x.spec+x.person+x.dept+x.cat).toLowerCase().includes(q.toLowerCase()));return <div className="content"><Toolbar q={q} setQ={setQ}/><Table headers={['日期','類別','品項','規格／序號','數量','人員','部門','動作','備註']} rows={filtered.map(x=><tr key={x.id}><td>{x.date}</td><td>{x.cat}</td><td>{x.item}</td><td>{x.spec||'-'}</td><td>{x.qty}</td><td>{x.person||'-'}</td><td>{x.dept||'-'}</td><td>{x.action}</td><td>{x.note||'-'}</td></tr>)}/></div>}
...
function ToolsInventoryPage({tools,onAdd,onDone}:{tools:Tool[];onAdd:()=>void;onDone:()=>void}){
  ...
  const template=()=>downloadInventoryTemplate('個人工具批次新增範本.xlsx',['電鑽','18V','電動工具','台','5','1','T-01','含電池'])
  const importFile=async(e:React.ChangeEvent<HTMLInputElement>)=>{const file=e.target.files?.[0];if(!file)return;setMsg('');try{const rows=await readInventoryFile(file);const data=rows.map(c=>({name:c[0],spec:c[1]||'',category:c[2]||'',unit:c[3]||'個',stock:Number(c[4]||0),safety_stock:Number(c[5]||0),location:c[6]||'',note:c[7]||'',serial:'',holder:'',department:'',issue_date:'',status:'在庫'}));const user=(await supabase.auth.getUser()).data.user;const {error}=await supabase.from('tools').insert(data.map(x=>({...x,owner_id:user!.id})));setMsg(error?error.message:`已批次新增 ${data.length} 筆個人工具`);if(!error)onDone()}catch(error:any){setMsg(error.message||'檔案無法讀取')}finally{e.target.value=''}}
  const toggle=(id:string)=>setSelected(selected.includes(id)?selected.filter(x=>x!==id):[...selected,id])
  const removeSelected=async()=>{if(!selected.length)return;if(!window.confirm(`確定刪除已勾選的 ${selected.length} 筆個人工具？`))return;const {error}=await supabase.from('tools').delete().in('id',selected);setMsg(error?error.message:`已刪除 ${selected.length} 筆個人工具`);if(!error){setSelected([]);onDone()}}
  const allSelected=rows.length>0&&rows.every(x=>selected.includes(x.id));const toggleAll=()=>setSelected(allSelected?selected.filter(id=>!rows.some(x=>x.id===id)):[...new Set([...selected,...rows.map(x=>x.id)])])
  return <div className="content">
    ...
    <Table headers={[<label className="selectAll"><input className="rowCheck" type="checkbox" checked={allSelected} onChange={toggleAll}/>全選</label>,'品項','規格','類別','庫存','安全庫存','位置','操作']} rows={rows.map(x=><tr key={x.id}>
      <td><input className="rowCheck" type="checkbox" checked={selected.includes(x.id)} onChange={()=>toggle(x.id)}/></td><td><b>{x.name}</b></td><td>{x.spec||'-'}</td><td>{x.category||'-'}</td><td>{Number(x.stock||0)} {x.unit||'個'}</td><td>{Number(x.safety_stock||0)} {x.unit||'個'}</td><td>{x.location||'-'}</td><td><button className="smallBtn" onClick={()=>setEditing(x)}>編輯</button></td>
    </tr>)}/>
...
function InventoryEditModal({kind,record,onClose,onDone}:{kind:'item'|'tool';record:Item|Tool;onClose:()=>void;onDone:()=>void}){
  const [f,setF]=useState({name:record.name||'',spec:record.spec||'',category:record.category||'',unit:record.unit||'個',stock:Number(record.stock||0),safety_stock:Number(record.safety_stock||0),location:record.location||'',note:record.note||''})
  const [msg,setMsg]=useState('');const [busy,setBusy]=useState(false)
  const save=async()=>{setMsg('');if(!f.name.trim())return setMsg('請輸入品項名稱');setBusy(true);const {error}=await supabase.from(kind==='item'?'items':'tools').update(f).eq('id',record.id);setBusy(false);if(error)return setMsg(error.message);onDone()}
  return <Modal title={`編輯${kind==='item'?'文具':'個人工具'}`} onClose={onClose}><FormFields f={f} setF={setF} fields={['name','spec','category','unit','stock','safety_stock','location','note']}/>{msg&&<div className="notice error">{msg}</div>}<button className="primary wide" onClick={save} disabled={busy}>{busy?'儲存中…':'儲存修改'}</button></Modal>
}
...
function InventoryLines({lines,setLines,items,tools}:{lines:InventoryLine[];setLines:(v:InventoryLine[])=>void;items:Item[];tools:Tool[]}){
  ...
  return <div className="multiLines">{lines.map((line,index)=>{const choices=(line.kind==='文具'?items:tools).map(x=>({id:x.id,label:`${x.name}｜${x.spec||'-'}｜庫存 ${x.stock||0}`}));const listId=`inventory-options-${line.key}`;return <div className="multiLine" key={line.key}>
    ...
    <Field label="品項"><input list={listId} value={line.itemQuery} placeholder="輸入品項名稱或規格搜尋" onChange={e=>{const value=e.target.value;const match=choices.find(x=>x.label===value);update(line.key,{itemQuery:value,itemId:match?.id||''})}}/><datalist id={listId}>{choices.map(x=><option key={x.id} value={x.label}/>)}</datalist></Field>
    <Field label="數量"><input type="number" min="1" value={line.qty} onChange={e=>update(line.key,{qty:Math.max(1,Number(e.target.value))})}/></Field>
