...
function ItemsPage({items,onDone}:{items:Item[];onDone:()=>void}){
  ...
  const template=()=>{const csv='名稱,規格,類別,單位,庫存,安全庫存,位置,備註\n原子筆,0.5mm,書寫用品,支,100,20,A-01,藍色';const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8'}));a.download='文具批次新增範本.csv';a.click();URL.revokeObjectURL(a.href)}
  const importFile=async(e:React.ChangeEvent<HTMLInputElement>)=>{const file=e.target.files?.[0];if(!file)return;setMsg('');if(!file.name.toLowerCase().endsWith('.csv')){setMsg('請使用下載的 Excel 相容 CSV 範本');return}const text=await readCsvText(file);const lines=text.replace(/^\ufeff/,'').split(/\r?\n/).filter(Boolean);const data=lines.slice(1).map(line=>{const c=line.split(',').map(x=>x.trim());return {name:c[0],spec:c[1]||'',category:c[2]||'',unit:c[3]||'個',stock:Number(c[4]||0),safety_stock:Number(c[5]||0),location:c[6]||'',note:c[7]||''}}).filter(x=>x.name);if(!data.length){setMsg('檔案內沒有可匯入的資料');return}const user=(await supabase.auth.getUser()).data.user;const {error}=await supabase.from('items').insert(data.map(x=>({...x,owner_id:user!.id})));setMsg(error?error.message:`已批次新增 ${data.length} 筆文具`);if(!error)onDone();e.target.value=''}
...
function ToolsInventoryPage({tools,onAdd,onDone}:{tools:Tool[];onAdd:()=>void;onDone:()=>void}){
  ...
  const template=()=>{const csv='名稱,規格,類別,單位,庫存,安全庫存,位置,備註\n電鑽,18V,電動工具,台,5,1,T-01,含電池';const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8'}));a.download='個人工具批次新增範本.csv';a.click();URL.revokeObjectURL(a.href)}
  const importFile=async(e:React.ChangeEvent<HTMLInputElement>)=>{const file=e.target.files?.[0];if(!file)return;setMsg('');if(!file.name.toLowerCase().endsWith('.csv')){setMsg('請使用下載的 Excel 相容 CSV 範本');return}const text=await readCsvText(file);const lines=text.replace(/^\ufeff/,'').split(/\r?\n/).filter(Boolean);const data=lines.slice(1).map(line=>{const c=line.split(',').map(x=>x.trim());return {name:c[0],spec:c[1]||'',category:c[2]||'',unit:c[3]||'個',stock:Number(c[4]||0),safety_stock:Number(c[5]||0),location:c[6]||'',note:c[7]||'',serial:'',holder:'',department:'',issue_date:'',status:'在庫'}}).filter(x=>x.name);if(!data.length){setMsg('檔案內沒有可匯入的資料');return}const user=(await supabase.auth.getUser()).data.user;const {error}=await supabase.from('tools').insert(data.map(x=>({...x,owner_id:user!.id})));setMsg(error?error.message:`已批次新增 ${data.length} 筆個人工具`);if(!error)onDone();e.target.value=''}
  const toggle=(id:string)=>setSelected(selected.includes(id)?selected.filter(x=>x!==id):[...selected,id])
