...
function ItemsPage({items,onDone}:{items:Item[];onDone:()=>void}){
  const [q,setQ]=useState('');const [msg,setMsg]=useState('');const [selected,setSelected]=useState<string[]>([])
  ...
  const receive=async(x:Item)=>{const raw=window.prompt(`請輸入「${x.name}」進貨數量`,'1');if(raw===null)return;const qty=Number(raw);if(!Number.isFinite(qty)||qty<=0)return setMsg('請輸入正確的進貨數量');const r=await supabase.rpc('receive_stationery',{p_item_id:x.id,p_quantity:qty,p_source:'進貨',p_note:''});setMsg(r.error?.message||`${x.name} 已進貨 ${qty} ${x.unit}`);if(!r.error)onDone()}
...
function ToolsInventoryPage({tools,onAdd,onDone}:{tools:Tool[];onAdd:()=>void;onDone:()=>void}){
  ...
  return <div className="content">
    ...
    <Table headers={['選取','品項','規格','類別','庫存','安全庫存','位置','操作']} rows={rows.map(x=><tr key={x.id}>
      <td><input className="rowCheck" type="checkbox" checked={selected.includes(x.id)} onChange={()=>toggle(x.id)}/></td><td><b>{x.name}</b></td><td>{x.spec||'-'}</td><td>{x.category||'-'}</td><td>{Number(x.stock||0)} {x.unit||'個'}</td><td>{Number(x.safety_stock||0)} {x.unit||'個'}</td><td>{x.location||'-'}</td><td><button className="smallBtn" onClick={()=>receive(x)}>進貨</button></td>
    </tr>)}/>
  </div>
}
