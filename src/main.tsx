  ...
  return <div className="appShell">
    ...
    <main className="main">
      ...
      {loading?<div className="loading">載入中…</div>:<>
        ...
        {tab==='in'&&<MultiReceivePage items={items} tools={tools} onDone={load}/>} 
...
function PeoplePage({people,onAdd,onDone}:{people:Person[];movements:Movement[];toolMovements:ToolMovement[];onAdd:()=>void;onDone:()=>void}){const [q,setQ]=useState('');const [editing,setEditing]=useState<Person|null>(null);const rows=people.filter(x=>(x.name+x.department).includes(q)).sort((a,b)=>(b.created_at||'').localeCompare(a.created_at||''));return <div className="content"><Toolbar q={q} setQ={setQ} add={onAdd} label="新增人員"/><Table headers={['姓名','部門','狀態','到職日','操作']} rows={rows.map(x=><tr key={x.id}><td><b>{x.name}</b></td><td>{x.department||'-'}</td><td>{x.active?'在職':'停用'}</td><td>{x.created_at?new Date(x.created_at).toLocaleDateString('zh-TW'):'-'}</td><td><button className="smallBtn" onClick={()=>setEditing(x)}>編輯人員資料</button></td></tr>)}/>{editing&&<EditPersonModal person={editing} onClose={()=>setEditing(null)} onDone={()=>{setEditing(null);onDone()}}/>}</div>}
...
function ToolsInventoryPage({tools,onAdd,onDone}:{tools:Tool[];onAdd:()=>void;onDone:()=>void}){
  ...
  const receive=async(x:Tool)=>{
    ...
    const {error}=await supabase.rpc('receive_tool_inventory',{p_tool_id:x.id,p_quantity:qty,p_note:'進貨'})
  ...
  return <div className="content">
    ...
    <Table headers={['品項','規格','類別','庫存','安全庫存','位置','操作']} rows={rows.map(x=><tr key={x.id}>
      <td><b>{x.name}</b></td><td>{x.spec||'-'}</td><td>{x.category||'-'}</td><td>{Number(x.stock||0)} {x.unit||'個'}</td><td>{Number(x.safety_stock||0)} {x.unit||'個'}</td><td>{x.location||'-'}</td><td><button className="smallBtn" onClick={()=>receive(x)}>進貨</button></td>
    </tr>)}/>
  </div>
}
...
function MultiIssuePage({items,tools,people,onDone}:{items:Item[];tools:Tool[];people:Person[];onDone:()=>void}){
  ...
  return <div className="content"><div className="panel formPanel"><h2>領用</h2><InventoryLines lines={lines} setLines={setLines} items={items} tools={tools}/><button className="smallBtn addLineBtn" onClick={()=>setLines([...lines,newLine()])}><Plus size={16}/>加入品項</button><div className="formGrid multiMeta"><Field label="領用人員"><select value={person} onChange={e=>{setPerson(e.target.value);setDept(people.find(x=>x.name===e.target.value)?.department||'')}}><option value="">請選擇</option>{people.filter(x=>x.active).map(x=><option key={x.id} value={x.name}>{x.name}｜{x.department}</option>)}</select></Field><Field label="部門"><input value={dept} onChange={e=>setDept(e.target.value)}/></Field><Field label="備註"><input value={note} onChange={e=>setNote(e.target.value)}/></Field></div>{msg&&<div className={msg.includes('完成')?'notice':'notice error'}>{msg}</div>}<button className="primary wide" onClick={submit} disabled={busy}>{busy?'處理中…':'確認領用'}</button></div></div>
}

function MultiReceivePage({items,tools,onDone}:{items:Item[];tools:Tool[];onDone:()=>void}){
  const [lines,setLines]=useState<InventoryLine[]>([newLine()]);const [source,setSource]=useState('退還');const [note,setNote]=useState('');const [msg,setMsg]=useState('');const [busy,setBusy]=useState(false)
  const submit=async()=>{setMsg('');const valid=lines.filter(x=>x.itemId);if(!valid.length)return setMsg('請至少選擇一筆品項');setBusy(true);for(const line of valid){const r=line.kind==='文具'?await supabase.rpc('receive_stationery',{p_item_id:line.itemId,p_quantity:line.qty,p_source:source,p_note:note}):await supabase.rpc('receive_tool_inventory',{p_tool_id:line.itemId,p_quantity:line.qty,p_note:note||source});if(r.error){setBusy(false);return setMsg(r.error.message)}}setBusy(false);setMsg(`已完成 ${valid.length} 筆退還／入庫`);setLines([newLine()]);setNote('');onDone()}
  return <div className="content"><div className="panel formPanel"><h2>退還／入庫</h2><InventoryLines lines={lines} setLines={setLines} items={items} tools={tools}/><button className="smallBtn addLineBtn" onClick={()=>setLines([...lines,newLine()])}><Plus size={16}/>加入退還／入庫品項</button><div className="formGrid multiMeta"><Field label="來源／動作"><select value={source} onChange={e=>setSource(e.target.value)}><option value="退還">退還</option><option value="採購入庫">採購入庫</option><option value="盤點調整">盤點調整</option></select></Field><Field label="備註"><input value={note} onChange={e=>setNote(e.target.value)}/></Field></div>{msg&&<div className={msg.includes('完成')?'notice':'notice error'}>{msg}</div>}<button className="primary wide" onClick={submit} disabled={busy}>{busy?'處理中…':'確認退還／入庫'}</button></div></div>
}
