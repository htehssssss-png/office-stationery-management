...
type InventoryLine={key:number;kind:'文具'|'個人工具';itemId:string;qty:number}
const newLine=(kind:'文具'|'個人工具'='文具'):InventoryLine=>({key:Date.now()+Math.random(),kind,itemId:'',qty:1})
function InventoryLines({lines,setLines,items,tools}:{lines:InventoryLine[];setLines:(v:InventoryLine[])=>void;items:Item[];tools:Tool[]}){
  ...
  return <div className="multiLines">{lines.map((line,index)=><div className="multiLine" key={line.key}>
    <span className="lineNumber">{index+1}</span>
    <Field label="類型"><select value={line.kind} onChange={e=>update(line.key,{kind:e.target.value as '文具'|'個人工具',itemId:''})}><option>文具</option><option>個人工具</option></select></Field>
    <Field label="品項"><select value={line.itemId} onChange={e=>update(line.key,{itemId:e.target.value})}><option value="">請選擇</option>{line.kind==='文具'?items.map(x=><option key={x.id} value={x.id}>{x.name}｜{x.spec}｜庫存 {x.stock}</option>):tools.map(x=><option key={x.id} value={x.id}>{x.name}｜{x.spec}｜庫存 {x.stock||0}</option>)}</select></Field>
